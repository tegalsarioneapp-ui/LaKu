...
router.get("/sync/status", async (_req, res) => {
    try {
        const result = await pool.query(`SELECT updated_at, version FROM bop_data WHERE rt_key = $1`, [RT_KEY]);
    ...
    try {
       ...
       ON CONFLICT (rt_key) DO UPDATE
         SET data       = EXCLUDED.data,
             ...
             updated_at = NOW()
       RETURNING updated_at, version`, [RT_KEY, JSON.stringify(body.data)]);
        return res.json({
            success: true,
