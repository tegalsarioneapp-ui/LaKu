...
router.post("/db/bop-sync", async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO bop_snapshots (data, label) VALUES ($1, $2) RETURNING id, created_at`, [JSON.stringify(data), label || "auto"]);
...
router.post("/db/photos", async (req, res) => {
    try {
        const { photos, activityName } = req.body;
        ...
        try {
            ...
            for (const p of photos) {
                ...
                await client.query(`INSERT INTO moku_photos
